...
namespace TheHero.Editor
{
    public static class TheHeroFixCameraAndHeroFollow
    {
        ...
        public static void FixCameraAndHeroFollow()
        {
            ...
            report.ValidationFails = ValidateCameraSetup(camera, hero.transform);
            WriteReport(report);
        ...
        private static THStrictGridHeroMovement FixHeroVisualsAndMovement(GameObject hero, FixReport report)
        {
            ...
            foreach (var other in hero.GetComponents<MonoBehaviour>())
            {
                ...
                if (typeName == "THGuaranteedHeroMovement" ||
                    typeName == "THReliableHeroMovement" ||
                    typeName == "THHeroMover")
                {
                    other.enabled = false;
                }
            }
        ...
        {
            return UnityEngine.Object.FindObjectsByType<GameObject>(inactiveMode)
        ...
        private static int ValidateCameraSetup(Camera camera, Transform hero)
        {
            int failures = 0;
            if (hero == null || hero.GetComponent<THStrictGridHeroMovement>() == null)
                failures++;

            var follow = camera != null ? camera.GetComponent<THCameraFollow>() : null;
            if (camera == null || !camera.orthographic || follow == null || follow.Target != hero)
                failures++;
